...
public class matrix_producer implements Runnable {
...

	{
		...
		/*
		 * while (!isOpen) { cap = new VideoCapture(0); isOpen = cap.open(0);
		 * System.out.println("Try to open times: " + ++count); }
		 * 
		 * if (!isOpen) { System.out.println("not open the stream!"); return; }
		 * 
		 */
		...
		while (true) {
			...
			// byte[] frameArray = new byte[((int) frame.total() *
			// frame.channels())];
			// frame.get(0, 0, frameArray);

			byte[] byteArray = new byte[ElementsSize];

			for (int i = 0; i < ElementsSize; i++) {
				byteArray[i] = (byte) (ElementsSize % 128);
			}
